...
emailEvent.on("sendEmail", async (email, username) => {
  ...
  const hashedOtp = await hash({ plainText: otp, salt: parseInt(process.env.SALT) });
await User.updateOne({ email }, {otp: hashedOtp});
  await sendEmails({ to: email, subject: subject.verify, html: `<h1>Hi ${username}</h1><p>Your OTP is ${otp}</p>` });
});
...
emailEvent.on("forgetPassword", async (email, username) => {
  ...
  const hashedOtp = await hash({ plainText: otp, salt: parseInt(process.env.SALT) });
await User.updateOne({ email }, {otp: hashedOtp});
  await sendEmails({ to: email, subject: subject.forgotPassword, html: `<h1>Hi ${username}</h1><p>Your OTP is ${otp}</p>` });
});
